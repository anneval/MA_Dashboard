...
# <font size="3">Expanding window OLS</font> {data-navmenu="Nowcasting"}
...
```{r}
Data <-"/Users/annevalder/Desktop/Uni/WU WIEN/SoSe_21/MA/GIT/All.RData"
load(Data)
```
...
### {style="border: 0px; background: none"}
 ...
 and to rely more on a data-driven approach while analyzing the nowcasting performance of the DESI in a multivariate time series setting, I extend the analysis by constructing a VAR model. 
 
\begin{equation}
\label{eq8}
    \mathbf{y_{t}} = \mathbf{c} + A_1 \mathbf{y_{t-1}} + ... + A_p \mathbf{y_{t-p}} + \boldsymbol{\epsilon_t}, \qquad \epsilon_t \sim \mathcal{N}_M(0,\,\Sigma)\ ,  \quad A_i(i = 1, \dotsc, p)
\end{equation}

Vector Autoregressive Model with 2 lags, where: $\mathbf{y_{t}}= [GDP_t, CCI_t, DESI_t]$
...
library(stargazer)
stargazer(VAR1_est$varresult, type = "html", header = F, title = "VAR(2)",  
...
#tests on the independence (i.e., autocorrelation), homoscedasticity (constant residual variance), and normality of the residuals. First, to test for autocorrelation in the model's residuals, I use the Portmanteau- and Breusch-Godfrey tests for serial correlation. Both tests reject the Null-hypothesis of no-autocorrelation, which implies that the residuals are independent. Next, I test for homoscedasticity using the ARCH-LM test. Since the Null-hypothesis that the squared residuals are a sequence of white noise cannot be rejected, I conclude that the residuals are homoscedastic. Last, I test if the residuals follow a normal distribution using the Jarque-Bera test. Due to a very small p-value, the Null-hypothesis, that the data is normally distributed is rejected. The cause for this is most likely the large outliers caused by the Covid19 crisis. Technically, the normality of residuals is desirable but not necessary, especially as the focus here is on prediction and not on inference. Violations of the normality condition can lead to problems when calculating confidence intervals and significance tests due to unreliable standard errors \parencite{wooldridge2012introductory}.\footnote{See Appendix section \ref{ModelResid} Table \ref{Modeldiagnostics} for detailed test results and figures.}
